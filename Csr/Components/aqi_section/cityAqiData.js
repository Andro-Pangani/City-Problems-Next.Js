...
function SubstanceBlock(props) {
  ...
  return (
    <ul className="substance_block" onClick={handleClick} key={props.index}>
 ...
 const showMarker = () => {
  ...
  return (
    <div className="aqi_station">
      <div className="aqi_station_header"
       onClick={() => {
      ...
      >{props.language == 'Georgian' ? props.data.st_full_address_ge : props.data.st_full_address_en}
      </div>
        ...
        {props.empty ? 
          ...
          <div className="no_data_section">
            <div className="no_data_text">
              {languages.aqiSection.recomendations[props.language].station_off.level}
            </div>  
          </div>
          )
            : (
...
const AqiStationConnected = connect(state => ({
...
function CityAqiData(props) {
  ...
  if (isLoading) {
    return (
      <div className="aqiDataLoading">Loading<div className="firstPreloader"><div className="secondPreloader"></div></div></div>
    );
  } else if (isLoaded) {
